...
<h2>ScreenQueueService</h2> <h3>Introduction</h3> <p>The <code>ScreenQueueService</code> class is responsible for managing a queue of screens in your Unity project. It utilizes the <strong>Zenject</strong> framework for dependency injection, <strong>UniTask</strong> for handling asynchronous operations, and <strong>SignalBus</strong> for communicating screen state changes. This service allows you to open screens in sequence, with options for automatic starting after adding screens to the queue.</p>

<figure>
    <img src="https://github.com/user-attachments/assets/3882229d-9894-4d3c-b1f6-9cf3f6779efb" alt="Condition System" width="500" /><br>
    <figcaption>Figure 5: Screen Queue Logic flowchart</figcaption>
</figure>

<h3>Installation</h3> <ol> <li><strong>Install the Module:</strong><br> To add the ScreenQueueService to your project, include it in your Unity project by copying the code into the appropriate namespace and adding dependencies for <strong>Zenject</strong>, <strong>Cysharp.Threading.Tasks</strong> (UniTask), and other GameFoundation scripts. </li> <li><strong>Register the ScreenQueueService:</strong><br> Add the following code to your <code>GameProjectInstaller</code> or equivalent MonoInstaller to bind <code>ScreenQueueService</code> to the Zenject container: </li> </ol> <pre><code class="language-csharp">public class GameProjectInstaller : MonoInstaller { public override void InstallBindings() { Container.Bind<ScreenQueueService>().AsSingle(); } }</code></pre> <h3>Function Documentation</h3> <h4><code>AddScreenToQueue&lt;TScreenPresenter&gt;()</code></h4> <p>Adds a screen to the queue, represented by its presenter. When this function is called, the screen is added to the queue for sequential display.</p> <ul> <li><strong>Input:</strong> <code>TScreenPresenter</code> - The presenter type implementing <code>IScreenPresenter</code>.</li> <li><strong>Output:</strong> None</li> <li><strong>Behavior:</strong> If <code>AutomaticStartAfterAddScreenToQueue</code> is enabled, the queue execution begins immediately.</li> </ul> <h4><code>AddScreenToQueue&lt;TScreenPresenter, TScreenModel&gt;(TScreenModel model)</code></h4> <p>Adds a screen to the queue with a model, represented by its presenter and the model. When called, the screen is added to the queue, and the provided model is used to initialize the screen.</p> <ul> <li><strong>Input:</strong> <ul> <li><code>TScreenPresenter</code> - The presenter type implementing <code>IScreenPresenter&lt;TScreenModel&gt;</code>.</li> <li><code>TScreenModel</code> - The model object used by the screen.</li> </ul> </li> <li><strong>Output:</strong> None</li> <li><strong>Behavior:</strong> If <code>AutomaticStartAfterAddScreenToQueue</code> is enabled, the queue execution begins immediately.</li> </ul> <h4><code>StartExecuteQueue()</code></h4> <p>Starts executing the queued screens in sequence. This method will process all screens in the queue one by one until the queue is empty.</p> <ul> <li><strong>Input:</strong> None</li> <li><strong>Output:</strong> None</li> <li><strong>Behavior:</strong> This method is invoked automatically if <code>AutomaticStartAfterAddScreenToQueue</code> is enabled and screens are added to the queue.</li> </ul>
